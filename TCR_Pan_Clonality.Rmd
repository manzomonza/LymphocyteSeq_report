...
T-Zell-Rezeptor-gamma: kein Nachweis einer T-Zellklonalität/ dominantes Rearrangement mit `r clo %>% filter(Rank == 1 ) %>% summarize(Frequency*100) %>% mutate(round(., 1))`% Frequenz vor polyklonalem Hintergrund

T-Zell-Rezeptor-beta: kein Nachweis einer T-Zellklonalität/ dominantes Rearrangement mit `r clob %>% filter(Rank == 1 ) %>% summarize(Frequency*100) %>% mutate(round(., 1))`% Frequenz vor polyklonalem Hintergrund
