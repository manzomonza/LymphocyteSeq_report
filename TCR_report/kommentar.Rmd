...
`r clone_i_freq(clonsum_gamma, i = 1)` für T-Zell-Rezeptor-gamma und `r clone_i_freq(clonsum_beta, i = 1)` für T-Zell-Rezeptor-beta nachgewiesen. Das Verhältnis zwischen dem dominanten Rearrangement und dem Hintergrund beträgt `r top_ratio_to_x(clonsum_gamma, j = 5)`x (gamma) und `r top_ratio_to_x(clonsum_gamma, j = 5)` (beta). 
